            ...
            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'user' ? (
                ...
                <div className="bg-white rounded-xl p-3 shadow-sm max-w-xs md:max-w-md lg:max-w-lg border border-gray-200">
                 ...
                 // --- AI/Model Message (Rendered as Markdown) ---
                <div className={`max-w-xl relative group ${msg.content.startsWith('Error:') ? 'text-red-600' : ''}`}>
                   {/* Apply className to a wrapper div, NOT ReactMarkdown */}
                  <div className="text-sm leading-relaxed prose prose-sm max-w-none">
                      <ReactMarkdown>
                          {msg.content}
                      </ReactMarkdown>
                  </div>
                  {/* Optional: Keep hover effect only if needed */}
           ...
           {error && !isLoading && ( // Only show error if not currently loading
             <div className="flex justify-start">
                 <div className="max-w-xl text-red-600">
                    {/* Apply className to a wrapper div for error message too */}
                    <div className="text-sm leading-relaxed prose prose-sm max-w-none prose-red">
                         <ReactMarkdown>
                             {`Error: ${error}`}
                         </ReactMarkdown>
                     </div>
